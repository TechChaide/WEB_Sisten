...
namespace Sisten_Chaide.Data
{
    public class DeliveryProvider
    {
        ...
        //nuevo feriados
        public  List<Feriados> consultaferiados()
        {

            Feriados retorno;
            Feriados consulta = new Feriados();
            List<Feriados> listretorno = new List<Feriados>();
            /////////////
            using (SqlCommand cmd = new SqlCommand())
            {
                cmd.Connection = _connection;
                cmd.CommandType = CommandType.StoredProcedure;
                cmd.CommandText = "[consulta_fecha_Feriados]";
                    ...
                    else
                    {
                        g = new GenerateDelivery()
                        {
                            ...
                            client = new Client()
                            {
                                ...
                                PhoneNumber = r["Telefono"].ToString(),
                                CellPhoneNumber = r["Celular"].ToString(),
                                Coordenadas = r["coordenadaslatlon"].ToString(),
                            },
                        };
                    }
                      delivery = g;
        ...
        public Collection<ItemProduct> Get(long orderId)
        {
            ...
            _conn.Open();
            string x2 = "CALL  \"SAPABAP1\".\"ZSPConsultaItem\" ('" + orderId + "');";
        ...
        public int AddDelivery(GenerateDelivery OrderDelivery, long userId)
        {
            ...
            if (error == 0)
            {
                ...
                // ========== DEBUG LOGGING ==========
                System.Diagnostics.Debug.WriteLine("=== AddDelivery DEBUG ===");
                System.Diagnostics.Debug.WriteLine("OrderNumber: " + (OrderDelivery.OrderNumber ?? "NULL"));
                System.Diagnostics.Debug.WriteLine("Zone Id: " + (OrderDelivery.driver?.zone?.Id ?? -999));
                System.Diagnostics.Debug.WriteLine("dateDelivery recibida: [" + (OrderDelivery.dateDelivery ?? "NULL") + "]");
                System.Diagnostics.Debug.WriteLine("EndDelivery recibida: [" + (OrderDelivery.EndDelivery ?? "NULL") + "]");
                System.Diagnostics.Debug.WriteLine("driver.user.Id: " + (OrderDelivery.driver?.user?.Id ?? -999));
                ...
                { 
                    ...
                    System.Diagnostics.Debug.WriteLine("ZONA RESIFLEX DETECTADA: usando fechas placeholder");
                }
                else
                {
                    try
                    {
                        if (string.IsNullOrEmpty(OrderDelivery.dateDelivery))
                        {
                            System.Diagnostics.Debug.WriteLine("ERROR: dateDelivery es NULL o vacío!");
                            date = DateTime.Now;
                        }
                        else
                        {
                            OrderDelivery.dateDelivery = OrderDelivery.dateDelivery.Replace("/", "-");
                            System.Diagnostics.Debug.WriteLine("Intentando parsear dateDelivery: [" + OrderDelivery.dateDelivery + "]");
                            date = DateTime.ParseExact(OrderDelivery.dateDelivery, "yyyy-MM-dd HH:mm:ss", CultureInfo.InvariantCulture);
                            System.Diagnostics.Debug.WriteLine("✓ dateDelivery parseada correctamente: " + date.ToString("yyyy-MM-dd HH:mm:ss"));
                        }
                        
                        if (string.IsNullOrEmpty(OrderDelivery.EndDelivery))
                        {
                            System.Diagnostics.Debug.WriteLine("ERROR: EndDelivery es NULL o vacío!");
                            Enddate = DateTime.Now.AddMinutes(45);
                        }
                        else
                        {
                            System.Diagnostics.Debug.WriteLine("Intentando parsear EndDelivery: [" + OrderDelivery.EndDelivery + "]");
                            Enddate = DateTime.ParseExact(OrderDelivery.EndDelivery, "yyyy-MM-dd HH:mm:ss", CultureInfo.InvariantCulture);
                            System.Diagnostics.Debug.WriteLine("✓ EndDelivery parseada correctamente: " + Enddate.ToString("yyyy-MM-dd HH:mm:ss"));
                        }
                    }
                    catch (Exception parseEx)
                    {
                        System.Diagnostics.Debug.WriteLine("EXCEPCIÓN AL PARSEAR FECHAS: " + parseEx.Message);
                        System.Diagnostics.Debug.WriteLine("Stack: " + parseEx.StackTrace);
                        date = DateTime.Now;
                        Enddate = DateTime.Now.AddMinutes(45);
                    }
                }
                ...
                using (SqlCommand cmd = new SqlCommand())
                {
                    ...
                    // ========== LOGGING FINAL ==========
                    System.Diagnostics.Debug.WriteLine("=== PARÁMETROS QUE SE ENVÍAN AL SP ===");
                    System.Diagnostics.Debug.WriteLine("@date_delivery: " + date.Date.ToString("yyyy-MM-dd"));
                    System.Diagnostics.Debug.WriteLine("@hour_delivery: " + date.ToString("yyyy-MM-dd HH:mm:ss"));
                    System.Diagnostics.Debug.WriteLine("@EndDelivery: " + Enddate.ToString("yyyy-MM-dd HH:mm:ss"));
                    System.Diagnostics.Debug.WriteLine("@driver_id: " + (OrderDelivery.driver?.user?.Id ?? -999));
                    System.Diagnostics.Debug.WriteLine("@zone_id: " + (OrderDelivery.driver?.zone?.Id ?? -999));
                    System.Diagnostics.Debug.WriteLine("=== EJECUTANDO SP [web.delivery.add] ===");

                    try
                    {
                        cmd.Connection.Open();
                        cmd.ExecuteNonQuery();
                        System.Diagnostics.Debug.WriteLine("✓ SP ejecutado exitosamente");
         ...
         //   string x = OrderDelivery.EndDelivery.ToString();
            ...
            using (SqlCommand cmd = new SqlCommand())
                ...
                {
                    // Para zonas normales (no resiflex), no validar
                    if (OrderDelivery.driver.zone.Id.ToString() != "70" && OrderDelivery.driver.zone.Id.ToString() != "72")
                    {
                        return 0; // Permitir siempre para zonas normales
                    }
                    
                    // Para zonas resiflex (70, 72), hacer validación
                    int statusCode = 0;
                    try
                    {
                        if (string.IsNullOrEmpty(OrderDelivery.dateDelivery))
                        ...
                        using (SqlCommand cmd = new SqlCommand())
                        {
                            ...
                            try
                            {
                                cmd.Connection.Open();
                                SqlDataReader r = cmd.ExecuteReader();
                                if (r.HasRows && r.FieldCount > 0)
                                {
                                    while (r.Read())
                                    {
                                        try
                                        {
                                            statusCode = Convert.ToInt32(r[0]); // Usar índice en lugar de nombre
                                        }
                                        catch
                                        {
                                            statusCode = 0;
                    ...
                    {
                        System.Diagnostics.Debug.WriteLine("Error isExistDelivery: " + capEx.Message);
                        statusCode = 0;
                    }
                ...
                /// <summary>
                /// Obtiene los SLOTS de horarios disponibles para agendamiento automático
                /// Consulta el SP [B2B_NuevoAgendamiento] que genera slots con rotación automática de choferes
                /// 
                /// DESCRIPCIÓN:
                /// - Genera slots de 45 minutos para cada día (lunes a sábado)
                /// - Respeta horario de la zona (inicio-fin)
                /// - Excluye almuerzo (12:45-13:30)
                /// - Pre-asigna automáticamente el PRIMER chofer disponible
                /// - Retorna SOLO slots con disponibilidad real (no teóricos)
                /// 
                /// </summary>
                /// <param name="zoneId">ID de la zona (requerido, 1-80+)</param>
                /// <param name="startDate">Fecha inicio (NULL = hoy, formato YYYY-MM-DD)</param>
                /// <param name="days">Cantidad de días a consultar (1-30, default 15)</param>
                /// <returns>List{ScheduleSlot} - Slots disponibles ordenados por fecha/hora</returns>
                public List<ScheduleSlot> GetAvailableSlots(int zoneId, DateTime? startDate = null, int days = 15)
                {
                    List<ScheduleSlot> slots = new List<ScheduleSlot>();

                    // IMPORTANTE: Usar conexión a chaide_db (PRODUCCIÓN) para leer slots
                    // Los datos reales de agendamientos existen en chaide_db, no en chaide_db_test
                    string prodConnectionString = "Data Source=192.168.7.30;Initial Catalog=chaide_db;Persist Security Info=True;User ID=sisten;Password=sisten2016";
                    
                    using (SqlConnection prodConnection = new SqlConnection(prodConnectionString))
                    using (SqlCommand cmd = new SqlCommand())
                    {
                        cmd.Connection = prodConnection;
                        cmd.CommandType = CommandType.StoredProcedure;
                        cmd.CommandText = "[B2B_NuevoAgendamiento]";
                        cmd.CommandTimeout = 10; // Timeout optimizado
                        ...
                        try
                        {
                            ...
                            while (reader.Read())
                            {
                                // Mapeo directo de columnas SQL → ScheduleSlot
                                ScheduleSlot slot = new ScheduleSlot()
                                {
                                    DateKey = Convert.ToDateTime(reader["DateKey"]),
                                    HourStart = TimeSpan.Parse(reader["HourStart"].ToString()),
                                    HourEnd = TimeSpan.Parse(reader["HourEnd"].ToString()),
                                    AssignedDriverId = Convert.ToInt32(reader["AssignedDriverId"]),
                                    AssignedDriverName = reader["AssignedDriverName"].ToString(),
                                    AvailableDriverCount = Convert.ToInt32(reader["AvailableDriverCount"])
                                };
                        ...
                        {
                            System.Diagnostics.Debug.WriteLine($"Error en GetAvailableSlots: {ex.Message}");
                            throw;
                        }
                        finally
                        {
