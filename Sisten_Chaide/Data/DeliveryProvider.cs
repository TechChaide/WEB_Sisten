...
namespace Sisten_Chaide.Data
{
    public class DeliveryProvider
    {
        ...
        /// <summary>
        /// Obtiene los slots disponibles para la zona indicada utilizando el SP B2B_NuevoAgendamiento.
        /// </summary>
        /// <param name="zoneId">Identificador de la zona.</param>
        /// <param name="startDate">Fecha inicial opcional en formato DateTime.</param>
        /// <param name="days">Cantidad de días a consultar (default 15).</param>
        /// <returns>Listado de slots disponibles.</returns>
        public List<ScheduleSlot> GetAvailableSlots(int zoneId, DateTime? startDate = null, int days = 15)
        {
            ...
            using (var connection = new SqlConnection(ApplicationContext.DefaultConnection))
            using (var command = new SqlCommand("[B2B_NuevoAgendamiento]", connection))
            {
                command.CommandType = CommandType.StoredProcedure;
                command.CommandTimeout = 30;

                command.Parameters.Add("@ZoneId", SqlDbType.Int).Value = zoneId;
                command.Parameters.Add("@StartDate", SqlDbType.Date).Value = startDate.HasValue ? (object)startDate.Value.Date : DBNull.Value;
                command.Parameters.Add("@Days", SqlDbType.Int).Value = days;
                command.Parameters.Add("@ProductCode", SqlDbType.VarChar, 50).Value = DBNull.Value;
                command.Parameters.Add("@Quantity", SqlDbType.Int).Value = DBNull.Value;
                ...
                try
                {
                    ...
                    using (var reader = command.ExecuteReader())
                    {
                        while (reader.Read())
                        {
                            ...
                            {
                                DateKey = reader["DateKey"] != DBNull.Value ? Convert.ToDateTime(reader["DateKey"]) : DateTime.MinValue,
                                HourStart = reader["HourStart"] != DBNull.Value ? TimeSpan.Parse(reader["HourStart"].ToString()) : TimeSpan.Zero,
                                HourEnd = reader["HourEnd"] != DBNull.Value ? TimeSpan.Parse(reader["HourEnd"].ToString()) : TimeSpan.Zero,
                                AssignedDriverId = reader["AssignedDriverId"] != DBNull.Value ? Convert.ToInt32(reader["AssignedDriverId"]) : 0,
                                AssignedDriverName = reader["AssignedDriverName"].ToString(),
                                AvailableDriverCount = reader["AvailableDriverCount"] != DBNull.Value ? Convert.ToInt32(reader["AvailableDriverCount"]) : 0
                            };
                ...
                {
                    System.Diagnostics.Debug.WriteLine(string.Format("Error en DeliveryProvider.GetAvailableSlots: {0}", ex.Message));
                    throw;
                }
            }
        ...
        public List<AvailabilitySlot> GetConsolidatedScheduleByZone(int zoneId, DateTime startDate, int numberOfDays)
        {
            var schedule = new List<AvailabilitySlot>();
            using (SqlCommand cmd = new SqlCommand())
            {
                cmd.Connection = _connection;
                cmd.CommandType = CommandType.StoredProcedure;
                cmd.CommandText = "[web.delivery.get_consolidated_schedule_by_zone]";
                ...
                try
                {
                    ...
                    while (reader.Read())
                    {
                        schedule.Add(new AvailabilitySlot()
                        {
                            HorarioDisponible = Convert.ToDateTime(reader["HorarioDisponible"]),
                ...
                {
                    if (!ex.Data.Contains("ActionParameters")) ex.Data.Add("ActionParameters", JsonConvert.SerializeObject(new { zoneId, startDate, numberOfDays }));
                    throw;
                }
                finally
                {
        ...
        public long? GetFirstAvailableDriverInZone(int zoneId, DateTime scheduleTime)
        {
            long? driverId = null;
            using (SqlCommand cmd = new SqlCommand())
            {
                cmd.Connection = _connection;
                cmd.CommandType = CommandType.StoredProcedure;
                cmd.CommandText = "[web.delivery.get_first_available_driver_in_zone]";
                cmd.Parameters.AddWithValue("@ZoneID", zoneId);
                ...
                try
                {
                    ...
                    {
                        driverId = Convert.ToInt64(result);
                    }
                }
                catch (Exception ex)
                {
                    if (!ex.Data.Contains("ActionParameters")) ex.Data.Add("ActionParameters", JsonConvert.SerializeObject(new { zoneId, scheduleTime }));
                    throw;
                }
                finally
                {
