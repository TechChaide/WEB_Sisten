            ...
            $.ajax({
                ...
                success: (function (response) {
                    if (response.result.client != null) {
                        ...
                        $("#client_PhoneNumber").val(response.result.client.PhoneNumber);
                        $("#client_City").val(response.result.client.City);
                        ciudad = response.result.client.City;
                        $("#client_Address").val(response.result.client.Address);
                        ...
                        $("#ReferenceAddress").val(response.result.ReferenceAddress);
                        $("#client_Coordenadas").val(response.result.client.Coordenadas);
        ...
        $('#calendar').fullCalendar({          
            ...
            events:
                ...
                eventClick: function (calEvent, jsEvent, view) {
                    ...
                    if (features.find(x => x === 'DELI0005') != undefined) {
                        ...
                        for (var i = 0; i < arregloDatos.length; i++) {
                            //var elemento = arregloDatos[i].fecha+"";
                            //  var subcadena = elemento.substring(0, 10);
                            if (initialDay == arregloDatos[i].fecha) {
                                feriad = "si"
                            }
                        }
                        if (feriad == "si") {
                        //if (initialDay == "2023-09-28") {
                            $('#modalTitle').html("Número de Pedido");
                            $('#modalBody').html("No se pueden agendar entregas en feriados");
                            $('#fullCalModal').modal();
                            return;
                        }
                ...
                dayClick: function (date, jsEvent, view) {
                    ...
                    if (dateCompare.getDay() == 6 && dateCompare.getHours()>="16") {
                        $('#modalTitle').html("Número de Pedido");
                        $('#modalBody').html("Horario no disponible para Sábado");
                        $('#fullCalModal').modal();
                        return;
                    }
                        ...
                        $.ajax({
                            ...
                            url: ' ../../Delivery/Feriados', // URL del controlador y acción
                   ...
                   /// sirve para validar fechas festivas edison vilema septiembre-2023
                    var feriad = "no";
                    for (var i = 0; i < arregloDatos.length; i++) {
                         //var elemento = arregloDatos[i].fecha+"";
                       //  var subcadena = elemento.substring(0, 10);
                    if (initialDay == arregloDatos[i].fecha && arregloDatos[i].ciudad == ciudad) {
                            feriad = "si"
                        }
                    }
                   // if (initialDay == "2023-09-28") {
                ...
                eventDrop: function (event, delta, revertFunc) {
                    ...
                    var año = dateCompare.getFullYear();
                    var mes = (dateCompare.getMonth() + 1).toString().padStart(2, '0'); // Añade ceros a la izquierda si es necesario
                    var dia = dateCompare.getDate().toString().padStart(2, '0'); // Añade ceros a la izquierda si es necesario
                    ...
                    for (var i = 0; i < arregloDatos.length; i++) {
                        //var elemento = arregloDatos[i].fecha+"";
                        //  var subcadena = elemento.substring(0, 10);
                        if (fechaFormateada == arregloDatos[i].fecha) {
                            feriad = "si"
                        }
                    }
                    ...
                    //if (fechaFormateada == "2023-09-28") {
                //        $('#modalTitle').html("Número de Pedido");
                 //       $('#modalBody').html("No se pueden agregar eventos en feriados drop");
                 //       $('#fullCalModal').modal();
                  //      revertFunc();
                  //      return;
                  //  }
